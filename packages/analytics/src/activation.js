...
export class ActivationManager {
     ...
     */
    static async checkActivation(userId) {
        try {
     ...
     */
    static async trackFirstOrder(userId, orderId, orderValue, templateId) {
        await trackEvent({
            userId,
     ...
     */
    static async trackFirstPayment(userId, orderId, orderValue, paymentMethod, templateId) {
        await trackEvent({
            userId,
     ...
     */
    static async trackFirstCompletion(userId, orderId, templateId) {
        await trackEvent({
            userId,
     ...
     */
    static async triggerActivation(userId, templateId) {
        const status = await this.checkActivation(userId);
        const activationPath = [];
        if (status.firstOrderCreated)
            activationPath.push("first_order");
        if (status.firstPaymentRecorded)
            activationPath.push("first_payment");
        if (status.firstOrderCompleted)
            activationPath.push("first_completion");
        await trackEvent({
            userId,
...
export class TemplateActivationTracker {
     ...
     */
    static async trackSignal(userId, templateId, signal, metadata) {
        // Track as custom event with template context
        await trackEvent({
            userId,
            eventName: `template.${signal}`,
            properties: metadata || {},
            templateId,
        });
    }

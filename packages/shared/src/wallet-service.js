...
export class WalletService {
            ...
            const account = await tx.ledgerAccount.upsert({
                where: {
                    ownerId_type_currency: {
                        ownerId: ownerId, // Cast to avoid TS null check strictness
                        type: accountType,
                        currency: "NGN",
                    }
                },
    ...
    static async getBalance(ownerId, type = 'MERCHANT_AVAILABLE') {
    ...
    static async getSystemBalance(type) {
        if (!type.startsWith("SYSTEM"))
            throw new Error("Not a system account type");
        const account = await prisma.ledgerAccount.findUnique({
            where: {
                ownerId_type_currency: {
                    ownerId: null, // Cast to avoid TS null check strictness
                    type,
                    currency: "NGN"
                }
            }

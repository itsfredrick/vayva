...
export async function themeRoutes(server: FastifyInstance) {
  // --- Template Gallery ---
  server.get("/templates", async (req: unknown, _reply) => {
    return await ThemeController.listTemplates(req.query);
  });

  server.get("/templates/:key", async (req: unknown, _reply) => {
    const { key } = req.params;
    return await ThemeController.getTemplate(key);
  });

  // --- Merchant Theme ---
  server.get("/theme", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
    return await ThemeController.getMerchantTheme(storeId);
  });

  server.post("/theme/apply", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
  ...
  server.put("/theme/settings", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
    return await ThemeController.updateSettings(storeId, req.body);
  });

  server.post("/theme/publish", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
    const userId = req.headers["x-user-id"];

...
export async function onboardingRoutes(server: FastifyInstance) {
  ...
  server.post("/wizard/step", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
  ...
  // --- Checklist ---
  server.get("/checklist", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
    return await OnboardingController.getChecklist(storeId);
  });

  // --- Storefront ---
  server.get("/storefront", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];
    return await OnboardingController.getStorefrontSettings(storeId);
  });

  server.put("/storefront", async (req: unknown, _reply) => {
    const storeId = req.headers["x-store-id"];

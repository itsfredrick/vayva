...
export async function marketingRoutes(server: FastifyInstance) {
  // --- Discounts ---
  server.get("/discounts/rules", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.listDiscountRules(storeId);
  });

  server.post("/discounts/rules", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.createDiscountRule(storeId, req.body);
  });

  server.post("/discounts/coupons", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
  ...
  // --- Segments ---
  server.get("/segments", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.listSegments(storeId);
  });

  server.post("/segments", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.createSegment(storeId, req.body);
  });

  // --- Campaigns ---
  server.get("/campaigns", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.listCampaigns(storeId);
  });

  server.post("/campaigns", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
  ...
  // --- Automations ---
  server.get("/automations", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    return await MarketingController.listAutomationRules(storeId);
  });

  server.put("/automations/:key", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];

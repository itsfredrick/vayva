...
export async function billingRoutes(server: FastifyInstance) {
  ...
  // --- Entitlements ---
  server.get("/entitlements/:feature", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
  ...
  server.get("/usage/:key", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    const { key } = req.params;
    return await BillingController.getUsageLimit(storeId, key);
  });

  server.post("/usage/:key/increment", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];
    const { key } = req.params;
  ...
  // --- Invoices ---
  server.get("/invoices", async (req: unknown, reply) => {
    const storeId = req.headers["x-store-id"];

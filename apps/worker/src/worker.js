...
async function start() {
        ...
        try {
            ...
            const message = await prisma.message.create({
                data: {
                    ...
                    type: messageData.type?.toUpperCase() || MessageType.TEXT,
    ...
    new Worker(QUEUES.AGENT_ACTIONS, async (job) => {
        ...
        if (!store)
            return;
        // 2. Get AI Response
        const replyText = await AIProvider.chat([{ role: "user", content: message.textBody }], {
     ...
     */
    new Worker("reconciliation", async (job) => {
     ...
     */
    new Worker(QUEUES.CHINA_CATALOG_SYNC, async (job) => {

...
export async function POST(req, { params }) {
    try {
        ...
        if (!store.payoutsEnabled) {
            return NextResponse.json({ error: "Payouts are already disabled for this store" }, { status: 400 });
        }
        // Disable payouts
        ...
        if (error.message?.includes("Insufficient permissions")) {
            return NextResponse.json({ error: "Insufficient permissions. SUPERVISOR role required." }, { status: 403 });
        }

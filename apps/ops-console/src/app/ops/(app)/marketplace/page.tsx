        ...
        <div className="p-8 space-y-6">
            ...
            {activeTab === "TEMPLATES" && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {templates?.map((t: any) => (
                        <div key={(t as any).id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-4">
                            <div className="flex justify-between items-start">
                                <h3 className="font-bold text-lg text-gray-900">{(t as any).name}</h3>
                                {(t as any).isFeatured && (
                            ...
                            <p className="text-sm text-gray-500 line-clamp-2 min-h-[40px]">{(t as any).description}</p>
                            <div className="flex flex-wrap gap-2">
                                {(t as any).tags.map((tag: any) => (
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                                <div className="flex items-center gap-2">
                                    <span className={`w-2 h-2 rounded-full ${(t as any).isActive ? 'bg-green-500' : 'bg-gray-300'}`} />
                                    <span className="text-xs font-medium text-gray-600">{(t as any).isActive ? "Active" : "Hidden"}</span>
                                </div>
                                <div className="flex gap-2">
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        onClick={() => toggleActive((t as any).id, (t as any).isActive)}
                                        className="p-1.5 text-gray-500 hover:bg-gray-100 rounded h-8 w-8"
                                    >
                                        {(t as any).isActive ? <Eye size={16} /> : <EyeOff size={16} />}
                                    </Button>
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        onClick={() => toggleFeatured((t as any).id, (t as any).isFeatured)}
                                        className={`p-1.5 rounded h-8 w-8 ${(t as any).isFeatured ? 'text-amber-500 hover:bg-amber-50' : 'text-gray-400 hover:bg-gray-100'}`}
                                    >
                                        <Star size={16} />
                                    </Button>
                                </div>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loadingApps ? (
                                ...
                                apps.map((app: any) => (
                                    <tr key={(app as any).id} className="hover:bg-gray-50">
                                        <td className="px-6 py-4 font-mono text-xs font-bold text-indigo-600">{(app as any).extensionId}</td>
                                        <td className="px-6 py-4 text-gray-900">{(app as any).developerId}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded text-xs font-bold ${(app as any).status === "ACTIVE" || (app as any).status === "PUBLISHED" ? "bg-green-100 text-green-700" :
                                                (app as any).status === "PENDING" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"
                                                }`}>
                                                {(app as any).status}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-gray-500 text-xs max-w-xs truncate" title={(app as any).manifestUrl}>
                                            {(app as any).manifestUrl}
                                        </td>
                                        <td className="px-6 py-4 text-gray-500 text-xs">
                                            {new Date((app as any).updatedAt).toLocaleDateString()}
                                        </td>
                                    </tr>
                                ))
                            )}

...
export default function StoreHome() {
    ...
    let activeTemplateId = store.theme.templateId;
    ...
    if (loadingProducts) {
        const themeColor = templateDef?.defaultTheme === "dark"
            ? "bg-[#0B0F19] text-white"
            : "bg-white text-gray-900";
        return (_jsx("div", { className: `min-h-screen flex items-center justify-center ${themeColor}`, children: "Loading..." }));
    }
    // If it's a specific layout (non-standard), render it securely
    if (componentKey !== "StoreShell" && ActiveLayout) {
        return _jsx(ActiveLayout, { store: store, products: products });
    }
    // DEFAULT STANDARD TEMPLATE (Fallback / StoreShell)
    // This is technically `vayva-standard`
    return (_jsxs(StoreShell, { children: [_jsx("section", { className: "relative px-4 py-20 bg-gray-50 mb-16", children: _jsxs("div", { className: "max-w-7xl mx-auto flex flex-col items-center text-center", children: [_jsx("h1", { className: "text-5xl md:text-7xl font-bold tracking-tight mb-6", children: store.name }), _jsx("p", { className: "text-lg md:text-xl text-gray-500 max-w-2xl mb-10 leading-relaxed", children: store.tagline ||
                                "Explore our varied collection of premium products." }), _jsx(Link, { href: `/collections/all?store=${store.slug}`, children: _jsx(Button, { className: "bg-black text-white px-8 py-4 rounded-full font-bold text-sm tracking-wide hover:bg-gray-900 transition-colors", children: "Shop All Products" }) })] }) }), _jsxs("section", { className: "max-w-7xl mx-auto px-4 mb-24", children: [_jsxs("div", { className: "flex items-center justify-between mb-8", children: [_jsx("h2", { className: "text-2xl font-bold", children: "Featured" }), _jsxs(Link, { href: `/collections/all?store=${store.slug}`, className: "flex items-center gap-2 text-sm font-medium hover:text-gray-600", children: ["View all ", _jsx(ArrowRight, { size: 16 })] })] }), loadingProducts ? (_jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10", children: [1, 2, 3, 4].map((i) => (_jsxs("div", { className: "animate-pulse", children: [_jsx("div", { className: "bg-gray-100 aspect-[4/5] rounded-xl mb-4" }), _jsx("div", { className: "h-4 bg-gray-100 w-2/3 rounded mb-2" }), _jsx("div", { className: "h-4 bg-gray-100 w-1/3 rounded" })] }, i))) })) : products.length > 0 ? (_jsx("div", { className: "grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10", children: products.slice(0, 4).map((product) => (_jsx(ProductCard, { product: product, storeSlug: store.slug }, product.id))) })) : (_jsx("div", { className: "text-center py-20 border border-dashed border-gray-200 rounded-xl", children: _jsx("p", { className: "text-gray-500", children: "No products available yet." }) }))] })] }));
}

        ...
        <div className="min-h-screen bg-[#FDFBF7] text-[#2C2C2C] font-serif">
            ...
            <nav className="border-b border-[#E5E1D8] px-8 py-6 flex justify-between items-center bg-white sticky top-0 z-50">
                ...
                <Button
                    ...
                    className="relative group p-0 h-auto rounded-full"
                        ...
                        {products.map((item) => (
                            <div key={item.id} className="group flex gap-6 cursor-pointer" onClick={() => addToCart(item)}>
                                ...
                                <div className="flex-1 py-2">
                                    ...
                                    <Button variant="link" className="text-xs font-bold uppercase tracking-widest border-b border-transparent hover:border-black transition-all p-0 h-auto">Add to Order</Button>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="fixed inset-0 z-[60] flex justify-end font-sans">
                    ...
                    <div className="relative w-full max-w-md bg-white h-full p-8 flex flex-col shadow-2xl animate-in slide-in-from-right">
                        <div className="flex justify-between items-center mb-10">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setIsCartOpen(false)} aria-label="Close cart side bar"><X className="w-6 h-6 hover:rotate-90 transition-transform" /></Button>
                        </div>
                                ...
                                <div key={item.id} className="flex gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Button variant="ghost" size="icon" onClick={() => updateQuantity(item.id, -1)} className="hover:bg-[#FDFBF7] p-1 rounded h-6 w-6" aria-label="Decrease quantity"><Minus className="w-3 h-3" /></Button>
                                            <span className="font-mono text-sm">{item.quantity}</span>
                                            <Button variant="ghost" size="icon" onClick={() => updateQuantity(item.id, 1)} className="hover:bg-[#FDFBF7] p-1 rounded h-6 w-6" aria-label="Increase quantity"><Plus className="w-3 h-3" /></Button>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="pt-8 border-t border-[#E5E1D8]">
                            ...
                            <Button
                                ...
                                className="w-full bg-[#2C2C2C] text-white py-6 h-auto rounded-full font-bold uppercase tracking-widest hover:bg-black transition-colors"
                            >
                                Confirm Order
                            </Button>
                        </div>

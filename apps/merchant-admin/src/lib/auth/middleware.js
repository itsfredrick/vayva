 ...
 */
export function withRBAC(arg1, arg2) {
    let handler;
    let permissions;
    ...
    return async (req, ...args) => {
        try {
            ...
            // Check permissions
            const hasPermission = permissions.length === 0 || permissions.some((p) => can(user.role, p));
            if (!hasPermission) {
...
export function withStoreAuth(handler, permissions = []) {

...
export class ResendEmailService {
    ...
    // --- 1. OTP Verification ---
    static async sendOTPEmail(to, code, firstName) {
    ...
    // --- 2. Welcome Email ---
    static async sendWelcomeEmail(to, firstName, storeName) {
        this.assertConfigured();
        try {
            const { data, error } = await this.client.emails.send({
    ...
    // --- 3. Password Changed ---
    static async sendPasswordChangedEmail(to) {
        this.assertConfigured();
        try {
            const { data, error } = await this.client.emails.send({
                from: this.fromEmail,
    ...
    // --- 4. Payment Receipt ---
    static async sendPaymentReceiptEmail(to, amountNgn, invoiceNumber, storeName) {
        this.assertConfigured();
        try {
            const { data, error } = await this.client.emails.send({
    ...
    static async sendSubscriptionExpiryReminder(to, storeName, planName, expiryDate) {
        this.assertConfigured();
        try {
    ...
    // --- 6. Order Shipped ---
    static async sendOrderShippedEmail(to, orderNumber, trackingUrl, storeName) {
        this.assertConfigured();
        try {
            const { data, error } = await this.client.emails.send({
                from: this.fromEmail,
     ...
     */
    static getOTPTemplate(code, firstName) {
        return `
    ...
    static getWelcomeTemplate(firstName, storeName) {
    ...
    static getReceiptTemplate(amount, invoiceRef, storeName) {
...
&nbsp;</p>
    ...
    static getShippedTemplate(orderNumber, trackingUrl, storeName) {
        return `
...
ResendEmailService.resendClient = null;
ResendEmailService.fromEmail = process.env.RESEND_FROM_EMAIL || `No-reply@${BRAND.domain}`;
ResendEmailService.billingEmail = process.env.EMAIL_BILLING || `Billing@${BRAND.domain}`;
ResendEmailService.helloEmail = process.env.EMAIL_HELLO || `Hello@${BRAND.domain}`;
ResendEmailService.supportEmail = process.env.EMAIL_SUPPORT || `Support@${BRAND.domain}`;

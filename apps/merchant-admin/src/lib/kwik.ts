...
export const KwikService = {
    ...
    async requestPickup(data: {
        pickup: { name: string, address: string, phone: string, email: string },
        delivery: { name: string, address: string, phone: string, email: string },
        parcel: { description: string, weight: number, isFragile: boolean }
    }) {
        const token = await getKwikToken();

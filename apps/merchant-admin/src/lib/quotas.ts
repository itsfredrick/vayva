...
const QUOTA_DEFAULTS: Record<QuotaType, number> = {
 ...
 */
export async function checkQuota(storeId: string, type: QuotaType): Promise<{
    allowed: boolean;
    remaining: number;
    limit: number;
 ...
 */
export async function incrementQuota(storeId: string, type: QuotaType): Promise<void> {
    if (!redis) return;

...
export async function verifyIdempotency(req) {
...
export async function saveIdempotencyResponse(key, response, bodyData // Pass explicit data since we can't easily read response stream twice
) {
    if (!key)
        return;

 ...
 */
export function sanitizeHTML(html, options = DEFAULT_CONFIG) {
    if (!html)
        return '';
 ...
 */
export function validateRedirectURL(url, allowedDomains = ['vayva.ng', 'vayva.com']) {
    if (!url)
        return null;
    try {
        ...
        const isAllowedDomain = allowedDomains.some(domain => parsed.hostname === domain || parsed.hostname.endsWith(`.${domain}`));
        if (isAllowedDomain)
            return url;
        return null;
    }

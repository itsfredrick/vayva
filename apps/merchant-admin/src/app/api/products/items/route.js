...
export const GET = withVayvaAPI(PERMISSIONS.COMMERCE_VIEW, async (request, { storeId }) => {
    try {
        ...
        const formattedProducts = products.map((product) => {
            const totalQuantity = product.inventoryItems?.reduce((sum, item) => sum + item.available, 0) || 0;
            return {
                id: product.id,
...
export const POST = withVayvaAPI(PERMISSIONS.COMMERCE_MANAGE, async (request, { storeId, user }) => {
    try {

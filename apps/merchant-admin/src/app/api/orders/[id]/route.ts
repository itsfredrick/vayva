...
export const GET = withVayvaAPI(PERMISSIONS.ORDERS_VIEW, async (req, { storeId, params }) => {
    try {
        const { id } = params;
...
export const PATCH = withVayvaAPI(PERMISSIONS.ORDERS_MANAGE, async (req, { storeId, user, params }) => {
    try {
        const { id } = params;

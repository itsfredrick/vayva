...
export const PATCH = withVayvaAPI(PERMISSIONS.MARKETING_MANAGE, async (req, { storeId, params }) => {
    try {
...
export const DELETE = withVayvaAPI(PERMISSIONS.MARKETING_MANAGE, async (req, { storeId, params }) => {
    try {
        const { id } = await params;
        // Validate ownership
        const existing = await prisma.flashSale.findUnique({

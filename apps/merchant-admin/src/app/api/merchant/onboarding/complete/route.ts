...
interface OnboardingState {
    template?: { id: string };
    finance?: { bankName?: string; accountNumber?: string; bvn?: string; accountName?: string; };
    logistics?: { deliveryMode?: string };
    business?: { businessRegistrationType?: string; name?: string; storeName?: string; slug?: string; cacNumber?: string; };
    identity?: { nin?: string; fullName?: string; cacNumber?: string; };
    kycStatus?: string;
    industrySlug?: string;
    ...
    try {
        ...
        if (onboarding?.data) {
            ...
            if (state.template?.id) {
                const profile = (ONBOARDING_PROFILES as any)[state.template.id];

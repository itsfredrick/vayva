...
export async function GET(request: unknown, { params }: unknown) {
    try {
        ...
        if (job.type === "withdrawals") {
            ...
            await logAuditEvent(user.storeId, user.id, AuditEventType.COMPLIANCE_REPORT_DOWNLOADED, {
        ...
        // 3. Audit Download
        await logAuditEvent(user.storeId, user.id, AuditEventType.EXPORT_DOWNLOADED, {
            targetType: "EXPORT_JOB",
            targetId: job.id,

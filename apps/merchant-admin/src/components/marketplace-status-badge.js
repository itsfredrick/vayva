...
export function MarketplaceStatusBadge({ status }) {
    const config = STATUS_CONFIG[status] || STATUS_CONFIG["Unlisted"];
    return (_jsx("span", { className: `px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${config.bg} ${config.text}`, children: status }));
}

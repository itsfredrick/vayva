...
export class OrderStateService {
     ...
     */
    static async transition(orderId, toStatus, actorId, storeId) {
        ...
        if (toStatus === FulfillmentStatus.OUT_FOR_DELIVERY) {
            if (order.customerEmail) {
                ...
                ResendEmailService.sendOrderShippedEmail(order.customerEmail, order.orderNumber, order.shipment?.trackingUrl ?? undefined, order.store.name).catch(e => console.error("Failed to send shipped email", e));
            }
        }
        // 5. Audit Log
        if (actorId) {
            await logAuditEvent(storeId, actorId, AuditEventType.ORDER_STATUS_CHANGED, {
                targetType: "ORDER",
                targetId: orderId,

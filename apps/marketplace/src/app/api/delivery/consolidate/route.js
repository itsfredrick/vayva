...
export async function POST(request) {
    try {
        ...
        const shouldConsolidate = await deliveryConsolidationService.shouldConsolidate(parentOrderId);
        ...
        const consolidatedFee = await deliveryConsolidationService.calculateConsolidatedFee(childOrders.map((o) => o.id));
        // Distribute fee across child orders
        await deliveryConsolidationService.distributeFee(parentOrderId, consolidatedFee);
        // Get breakdown for response
        const breakdown = await deliveryConsolidationService.getFeeBreakdown(parentOrderId);
        return NextResponse.json({
            success: true,
            consolidatedFee,
            breakdown,
        ...
        return NextResponse.json({ error: error.message || "Failed to consolidate delivery fee" }, { status: 500 });
    }
}
export async function GET(request) {
    try {
        ...
        return NextResponse.json({ error: error.message || "Failed to get delivery breakdown" }, { status: 500 });
    }
}

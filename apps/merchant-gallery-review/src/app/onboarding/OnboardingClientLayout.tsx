...
const STEPS: StepDef[] = [
  { id: "welcome", path: "/onboarding/welcome", label: "Industry & Path" },
  { id: "business", path: "/onboarding/business", label: "Business Profile" },
  {
    id: "communication",
    path: "/onboarding/communication",
    label: "Communication",
  },
  { id: "visuals", path: "/onboarding/visuals", label: "Visual Storefront" },
  { id: "finance", path: "/onboarding/finance", label: "Finance" },
  { id: "logistics", path: "/onboarding/logistics", label: "Logistics" },
  { id: "inventory", path: "/onboarding/inventory", label: "Products" },
...
export function OnboardingClientLayout({
  ...
  // Filter steps based on skippedSteps AND dynamic logic (e.g. logistics)
  const validSteps = STEPS.filter((step) => {
    // 1. Check explicit skipped steps array
    if (state?.skippedSteps?.includes(step.id)) return false;

    // 2. Check Dynamic Logic: Delivery
    if (step.id === "logistics" && state?.intent?.hasDelivery === false) {
      return false;
    }
